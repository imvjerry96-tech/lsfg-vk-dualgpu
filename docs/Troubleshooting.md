...
If vkb-vk does not seem to be doing *anything*:
...
- Install `vulkan-tools` and run `vulkaninfo | grep -i VK_LAYER_VKBPVK_frame_generation`.
...
- Launch the game with the environment variable `VK_LOADER_DEBUG=layer` set.
  - Look for lines mentioning `VK_LAYER_VKBPVK_frame_generation` inbetween `<Loader>` and `<Device>`.
  - If you can't find any, try again using `VKBPVK_ENV=1`.
    - If it still doesn't show up, you may be running in flatpak.
    - If it does show up, then the `active_in` property of your profile is likely misconfigured. Reconfigure it, then try again without `VKBPVK_ENV=1`.
- Check for warnings/errors from vkb-vk in the terminal/log output. These will often give clues as to what is going wrong.
- If there are no errors/warnings and you have gone through all above steps, then move onto the next section.

If vkb-vk is loaded, but frame generation is not working:
...
If games do not open at all with vkb-vk enabled for them (stuck at black screen):
- Ensure you configured the correct `gpu` for this profile, in case you have multiple GPUs and/or drivers (vkb-vk-ui will show all available GPUs in a dropdown), vkb-vk might be defaulting to a different one than the game is using
...
- The relevant section of your vkb-vk configuration file.
